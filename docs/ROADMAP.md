...
| Spatial-chaining fallback for unparseable buoy names (4% US / 13% NL) | `docs/SPEC-CHANNEL-AXES.md` §9 | done | pipeline | grouped by `CATLAM` + proximity (`SPATIAL_KEY`), ordered by nearest-neighbour walk, confidence -0.2 (lower than the existing bare-number fallback's -0.1) |
