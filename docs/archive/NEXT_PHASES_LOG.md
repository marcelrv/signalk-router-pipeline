# Next Phases — Navmesh-Hybrid Pipeline (Post Phase 0) — ARCHIVED CHRONOLOGICAL LOG

> Other docs named in this log (`PHASE_3_DESIGN.md`, `PHASE_4_DESIGN.md`, `STITCHING_DESIGN.md`, `README.md`) are at the repo root; `SPEC-*.md` are in `docs/`.
>
> **Archived 2026-09-21.** This is the full, finished chronological hardening log, moved here with
> `git mv` from the repo root (history preserved). Almost everything below is historical: the status
> statements in it describe the moment each was written, not today. What is still open now is in the
> short live file [`../../NEXT_PHASES.md`](../../NEXT_PHASES.md); the status table for the whole project is
> [`../ROADMAP.md`](../ROADMAP.md).
>
> **Status refresh (2026-09-21), verified against `git log` (HEAD `c8178cc`, PR #26):**
> - Phase 0, 1 and 2 are done. Rounds 3-25 are done, including the Zeeland bridge/lock/backlog series;
>   the Phase 2 hardening correctness backlog was declared cleared at Round 21 ("BACKLOG CLEARED",
>   further down).
> - The "not resolved" / "URGENT" texts in the opening block and in Rounds 5/6 below are **resolved
>   by Rounds 13-21**; they are kept as history.
> - "Uncommitted" claims for Rounds 13-15 (around the Round 13/14+15 headings) are stale: the
>   pipeline change is committed as `73c6492` and the documentation as `3f28a68`.
> - The open question at the end of the file (2026-08-14, adopted seam-node depths) was resolved by
>   commit `655ae47` ("Preserve adopted seam-node depths; fix a GEOS topology crash in deep-mask closing").
> - Seam stitching by registry (`seam_registry.py`) and the 19-region US East Coast rollout (builds
>   #13-#32) are done. Round 25 "grid-snap" (Chunk 1) was rejected and is dead.
> - Everything after this log (graph density, channel axes, graph cleanup) is tracked in
>   `docs/SPEC-*.md` and `data/BUILD_LOG.md`, not here.
...
> Archive note (2026-09-21): the "not resolved" text in this opening block was resolved by Rounds 13-21 (see "BACKLOG CLEARED" at Round 21).
...
> Archive note (2026-09-21): no longer urgent. Done, and superseded by Rounds 3-21 (backlog cleared at Round 21).
...
> Archive note (2026-09-21): resolved by Rounds 13-21 ("BACKLOG CLEARED", Round 21). Kept as history.
...
> Archive note (2026-09-21): "uncommitted" is stale. Committed as `73c6492` (code) and `3f28a68` (docs).
...
Checked against the actual (uncommitted; since committed, see the status refresh at the top of this file) code and generated databases
...
> Archive note (2026-09-21): whether this is still an open decision is tracked in `docs/ROADMAP.md`; the later BUILD_LOG builds #13-#32 used `*_reclip` inputs, so it may be partly overtaken. Not verified here.
...
> Archive note (2026-09-21): the open question below was resolved by commit `655ae47` ("Preserve adopted seam-node depths").
